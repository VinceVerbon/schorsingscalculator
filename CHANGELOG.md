...
### Added
- **Test-fixture kenteken `TEST-123`** — short-circuit in `fetchRDW`: voert de gebruiker `TEST-123` in, dan wordt de RDW-call overgeslagen en geeft de tool een ingebakken mock-record terug (`TEST_KENTEKEN_DATA`-constante). Bedoeld voor scenario-tests rond de schorsings-tariefgrenzen (3500 kg, 15 jaar) en MRB-randgevallen. Profielpaneel toont een gele `TEST`-banner zodat de fixture niet met echte RDW-data verward kan worden. Waarden zijn alleen aan te passen in de bron (geen UI-formulier — voorkomt vervuiling van echte profielen).
...
- **Tarief-zelftest** — 7 nieuwe `computeTariff`-fixtures voor de RDW-schorsingstarief-regel (Bijlage VII, BWBR0051875): kampeerwagen `<15`j ongeacht TMM → hoog (regressie-anker 4000 kg), kampeerwagen `≥15`j → laag, niet-kamper licht/zwaar/N-reg, en `isKamperOverride`-pad. Draait samen met de bestaande datum- en MRB-tests in het tabblad "Regels & uitleg".
...
- **Schorsingstarief — kampers `>3500` kg ten onrechte op laag tarief gezet** — root cause: `computeTariff()` paste de 3500 kg-grens toe op àlle voertuigen, terwijl *Regeling tarieven Dienst Wegverkeer 2026, Bijlage VII* (BWBR0051875) "Kampeerwagens" als **aparte categorie** noemt, parallel aan "voertuig licht/zwaar". Voor kampeerwagens telt UITSLUITEND de leeftijd (`<15`j → hoog, `≥15`j → laag); de 3500 kg-grens sorteert alleen niet-kampers. Gebruiker meldde dat een 4000 kg Dethleffs DET 2015 in de RDW-calculator €88,05 (HIGH) gaf terwijl onze tool €29,10 (LAAG) toonde. `computeTariff()` herschreven met een `isKamper`-tak (gebaseerd op `rdw.inrichting` of `isKamperOverride`); voor kampers geldt enkel `isOld`. De rule-tekst in tabblad "Regels & uitleg" is bijgewerkt met bronvermelding naar BWBR0051875. Regressie-anker toegevoegd aan de zelftest.
